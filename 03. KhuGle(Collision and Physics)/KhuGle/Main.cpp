...
CCollision::CCollision(int nW, int nH) : CKhuGleWin(nW, nH) 
{
	...
	m_pCircle2 = new CKhuGleSprite(GP_STYPE_ELLIPSE, GP_CTYPE_STATIC, CKgLine(CKgPoint(320, 300), CKgPoint(380, 360)),
		KG_COLOR_24_RGB(255, 0, 0), false, 100);
	m_pLine = new CKhuGleSprite(GP_STYPE_LINE, GP_CTYPE_STATIC, CKgLine(CKgPoint(0, 400), CKgPoint(640, 400)), 
	...
	for(auto &Layer : m_pScene->m_Children)
	{
		...
		for(auto &SpriteA : Layer->m_Children)
		{
			...
			for(auto &SpriteB : Layer->m_Children)
			{
				...
				if((((CKhuGleSprite *)Target)->m_nType == GP_STYPE_ELLIPSE) || (Target->m_nType == GP_STYPE_RECT))
				{
